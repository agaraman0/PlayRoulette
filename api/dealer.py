...
from api.dealer_helper import is_dealer_assigned, throw_number
from models import *
...
def settle_up_game(dealer_id):
    ...
    with LocalSession(Session) as session:
        filters = [Game.did == dealer_id, Game.status == GameStatus.OPEN.value]
        game_object = session.query(Game).filter(and_(*filters)).first()
        if game_object.id:
            game_object.number = game_number
            bets = session.query(Bet).join()
